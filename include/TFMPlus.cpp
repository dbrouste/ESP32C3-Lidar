...
//EspSoftwareSerial::UART MySerial1;
...
{
    pStream[id-1] = streamPtr;          // Save reference to stream/serial object.
    delay(10);                   // Delay for device data in serial buffer.
    if( (*pStream[id-1]).available())   // If data present...
    {
...
void TFMPlus::flush() {
    while ((*pStream[id-1]).available() > TFMP_FRAME_SIZE) {
        int bytesToRead = min((*pStream[id-1]).available() - TFMP_FRAME_SIZE, 64); // Adjust chunk size as needed
        uint8_t buffer[64];  // Temporary buffer to hold discarded data
        (*pStream[id-1]).readBytes(buffer, bytesToRead);
    }
}
...
bool TFMPlus::getData( int16_t &dist, int16_t &flux, int16_t &temp)
{
    ...
    //while( (*pStream[id-1]).available() > TFMP_FRAME_SIZE) (*pStream[id-1]).read();
    flush();
    ...
    {
        if( (*pStream[id-1]).available())
        {
            // Read one byte into the framebuffer's
            // last plus one position.
            frame[ TFMP_FRAME_SIZE] = (*pStream[id-1]).read();
...
bool TFMPlus::sendCommand( uint32_t cmnd, uint32_t param)
{
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    while( (*pStream[id-1]).available()) (*pStream[id-1]).read();  // flush input buffer
    (*pStream[id-1]).flush();                                // flush output buffer
    for( uint8_t i = 0; i < cmndLen; i++) (*pStream[id-1]).write( cmndData[ i]);
    ...
    while( ( reply[ 0] != 0x5A) || ( reply[ 1] != replyLen))
    {
        if( (*pStream[id-1]).available())
        {
            ...
            // last-plus-one position.
            reply[ replyLen] = (*pStream[id-1]).read();
            // Shift the last nine bytes one byte left.
...
void TFMPlus::setHightBaudRate()
{
    //    - - Set Baud rate - - - - -
    printf( "System Baud rate: ");
    if( sendCommand( SET_BAUD_RATE, BAUD_921600)) {
        printf( "%2ub/s.\r\n", BAUD_921600);
    } else printReply();
    if (id == 1) {
        Serial1.updateBaudRate(HIGHT_BAUD_RATE);
        //MySerial1.begin(HIGHT_BAUD_RATE);
    }
    if (id == 2) {
        MySerial2.begin(HIGHT_BAUD_RATE); //!!!!!!!
    }
}

void TFMPlus::init(int lidarId, boolean highRate, int rx, int tx) {
    Serial.print("InitLidar: ");Serial.println(lidarId);
    id = lidarId; // on set l'id du lidar
    if (id == 1) {
        Serial1.begin(DEFAULT_BAUD_RATE, SERIAL_8N1, rx, tx);
        //MySerial1.begin(DEFAULT_BAUD_RATE, SWSERIAL_8N1, rx, tx, false);
        delay(20); // Give port time to initalize
        begin(&Serial1);   // Initialize device library object and...
        //begin(&MySerial1); 
                             // pass device serial port to the object.
    }
    if (id == 2) {
        MySerial2.begin(DEFAULT_BAUD_RATE, SWSERIAL_8N1, rx, tx, false);
        if (!MySerial2) { // If the object did not initialize, then its configuration is invalid
        ...
        delay(20); // Give port time to initalize
        begin(&MySerial2);   // Initialize device library object and...
                             // pass device serial port to the object.
    }
    ...
    if( sendCommand( SYSTEM_RESET, 0))
    {
        ...
        delay(20); // Give port time to initalize
        Serial.println( "Retry System reset with higher baud rate: ");
        if( sendCommand( SYSTEM_RESET, 0))
        {
            Serial.println( "passed.\r\n");
        }
        if (id == 1) {
            Serial1.updateBaudRate(DEFAULT_BAUD_RATE);
            ///MySerial1.begin(DEFAULT_BAUD_RATE);
        }
        if (id == 2) {
            MySerial2.begin(DEFAULT_BAUD_RATE);
        }
    };
  ...
  // - - Display the firmware version - - - - - - - - -
    printf( "System Firmware version: ");
    if( sendCommand( OBTAIN_FIRMWARE_VERSION, 0))
    {
        printf( "%1u.", version[ 0]); // print three single numbers
        printf( "%1u.", version[ 1]); // each separated by a dot
        printf( "%1u\r\n", version[ 2]);
    }
    else printReply();

    // - - Set the data frame-rate to 1000Hz - - - - - - - -
    printf( "System Data-Frame rate: ");
    if( sendCommand( SET_FRAME_RATE, FRAME_1000))
    {
        printf( "%2uHz.\r\n", FRAME_1000);
    }
    else printReply();
