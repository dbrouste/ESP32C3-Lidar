...
void flushLidar() {
    lidar1.flush();
    Serial.println("Lidar 1 data flushed");
    if (SENSOR_NUMBER > 1)
    {
        lidar2.flush();
        Serial.println("Lidar 2 data flushed");
    }
}
...
void calibrateLidar() {
    ...
    if (minDistance1 < 0 or (minDistance2 < 0 and SENSOR_NUMBER > 1)) {
    ...
    Serial.println("calibrate Lidar OK");
}
...
void initLidar() {
    ...
    calibrateLidar();
    Serial.println("Init Lidar OK");
}

void initFlash() {
    // Initilise le flash sur éteind
    Serial.println("Lancement Init flash");
...
boolean isDistanceChange() {
    return distance1 <= minDistance1 or (distance2 <= minDistance2 and SENSOR_NUMBER > 1);
}
